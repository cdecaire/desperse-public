...
### Framework Detection
Check for config files and package.json dependencies (in priority order):

**TanStack Start** (check first - it uses Vite under the hood):
- `@tanstack/react-start` in package.json dependencies → **TanStack Start**
  - Uses file-based routing in `src/routes/`
  - Check for `@tanstack/react-router` for routing patterns
  - Check for `@tanstack/react-query` for data fetching patterns

**Other Frameworks:**
- `next.config.js` or `next.config.mjs` or `next.config.ts` → **Next.js**
  - Check for `app/` directory → App Router
  - Check for `pages/` directory → Pages Router
- `vite.config.js` or `vite.config.ts` (without TanStack Start) → **Vite**
...
- Tailwind v4: Check for `@tailwindcss/vite` in dependencies (no config file needed)
...
Check for pre-built component libraries:

**shadcn/ui:**
- `components.json` file with `"$schema": "https://ui.shadcn.com/schema.json"` → **shadcn/ui**
- `shadcn` in devDependencies → confirms shadcn CLI available
- Check `components.json` for:
  - `style`: "default" or "new-york" (affects component styling)
  - `aliases.ui`: location of UI components (typically `@/components/ui`)
  - `tailwind.cssVariables`: whether CSS variables are used
- Scan the UI components directory for existing primitives (Button, Card, Input, etc.)
- **IMPORTANT**: When generating variants, prefer using existing shadcn components over creating custom ones

**Radix UI (standalone):**
- `@radix-ui/*` packages without `components.json` → using Radix primitives directly
...
```json
{
  ...
  "framework": "tanstack-start|nextjs-app|nextjs-pages|vite|remix|nuxt|astro",
  "packageManager": "pnpm",
  "stylingSystem": "tailwind|tailwind-v4|mui|chakra|antd|styled-components|emotion|css-modules",
  "componentLibrary": {
    "name": "shadcn|radix|headlessui|react-aria|none",
    "style": "new-york|default",
    "uiPath": "@/components/ui",
    "existingComponents": ["Button", "Card", "Input", "Dialog", "Select"]
  }
}
```
...
**TanStack Start:**
Create `src/routes/__design_lab.tsx` with TanStack Router conventions:
```tsx
import { createFileRoute } from '@tanstack/react-router'
import DesignLab from '../.claude-design/lab/page'

export const Route = createFileRoute('/__design_lab')({
  component: DesignLab,
})
```
- TanStack Start uses file-based routing in `src/routes/`
- Route files export a `Route` constant created with `createFileRoute`
- The route path is derived from the file name (`__design_lab` → `/__design_lab`)
...
**shadcn/ui Usage (when detected):**
- Import components from the project's UI path (typically `@/components/ui/`)
- Use the shadcn component API (props like `variant`, `size`, `asChild`)
- Respect the detected style ("new-york" vs "default")
- Available components should be scanned from `components.json` aliases
- Example imports:
  ```tsx
  ...
  ```
- If a needed component doesn't exist, note it in the variant comments as a dependency
- Combine shadcn primitives with Tailwind utilities for layout and spacing

**TanStack-specific patterns (when detected):**
- Use TanStack Query patterns for data fetching in variants (useQuery, useMutation)
- Route components should be simple wrappers that render the variant
- Respect the project's server function patterns if present
...
- Remove temporary route files:
  - `src/routes/__design_lab.tsx` (TanStack Start)
  - `src/routes/__design_preview.tsx` (TanStack Start)
