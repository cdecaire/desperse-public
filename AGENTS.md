...
- Use the generic `kanban-pr-lifecycle` skill for code-changing Echoes website tasks on this repo. Its Desperse repository policy encodes the clone URL, branch workflow, allowed Echoes paths, PR label convention, and Kanban close-the-loop rules. Do not guess the owner/repo slug or improvise the GitHub workflow when a task touches the Echoes pages.
...
**Type checking:** `src/routeTree.gen.ts` is generated and **gitignored**, so a fresh checkout has no route tree and `npx tsc` will report ~70 phantom "not assignable to `undefined`" route errors. Generate it first:
```bash
pnpm install
pnpm routes:generate   # writes src/routeTree.gen.ts (also done automatically by pnpm dev/build)
npx tsc --noEmit       # should be 0 errors
pnpm test --run
```
Never commit `src/routeTree.gen.ts` — it's ignored and regenerated by the build.
