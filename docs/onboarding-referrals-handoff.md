...
3. **Direct welcome-URL attribution (fixed)**: `server/middleware/referral-welcome-attribution.ts` now establishes or restores the signed attribution cookie when a visitor opens `/i/<code>/welcome` directly. Invalid-code URLs (`?invalid=1`) and unresolved codes remain attribution-free. The normal `/i/<code>` and safe `?next=/post/<id>` redirect flows are unchanged.
