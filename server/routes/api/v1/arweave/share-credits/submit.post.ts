 ...
 *   "success": true,
 *   "data": {
 *     "approvalDataItemId": "arweave-tx-id",
 *     "approvedWincAmount": "1000000000000"
 *   }
 * }
 */
...
import { submitSignedShareCredits } from "@/server/services/arweave/credit-sharing";
...
export default defineEventHandler(async (event) => {
	...
	try {
		const result = await submitSignedShareCredits(sessionId, signatureBase64, userId);

		return {
			success: true,
			data: {
				approvalDataItemId: result.approvalDataItemId,
				approvedWincAmount: result.approvedWincAmount,
			},
			requestId,
		};
	} catch (err) {
		const message = err instanceof Error ? err.message : "Unknown error";
		console.error("[ArweaveShareSubmit] Failed:", message);
		...
		return {
			...
			error: {
				...
				message: isClientError ? message : "Failed to submit credit sharing",
			},
			requestId,
		};
	}
