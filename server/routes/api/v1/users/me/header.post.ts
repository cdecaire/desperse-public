 ...
 * Upload header background image for current user
 *
...
import { uploadHeaderDirect } from '@/server/utils/profile'
...
export default defineEventHandler(async (event) => {
	...
	try {
		...
		const result = await uploadHeaderDirect(token, {
		...
		console.error('[POST /users/me/header] Error:', error)
		event.node!.res!.statusCode = 500
		return {
			success: false,
			error: { code: 'internal_error', message: 'Failed to upload header image' },
			requestId,
		}
	}
})
