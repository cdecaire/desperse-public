...
export const checkPurchaseStatus = createServerFn({
  ...
  try {
    ...
    // Handle awaiting_fulfillment and master_created - delegate to fulfillPurchaseDirect
    // (fulfillPurchaseDirect includes Arweave finalization for permanent storage editions)
    if (purchase.status === 'awaiting_fulfillment' || purchase.status === 'master_created') {
      console.log(`[checkPurchaseStatus] Delegating to fulfillPurchaseDirect for ${purchaseId} (status: ${purchase.status})`);
