...
// Copy of generateNftMetadata (same as in create-post.ts)
function generateNftMetadata(post: {
  id: string; caption: string | null; mediaUrl: string; coverUrl: string | null
  type: 'collectible' | 'edition'; maxSupply: number | null; price: number | null
  currency: 'SOL' | 'USDC' | null; nftName?: string | null; nftSymbol?: string | null
  nftDescription?: string | null; sellerFeeBasisPoints?: number | null
  isMutable?: boolean | null; categories?: Category[] | null
  protectDownload?: boolean; assetId?: string
  assets?: Array<{ id: string; url: string; mimeType: string; isPreviewable: boolean }>
}, creator: { displayName: string | null; usernameSlug: string; walletAddress: string }) {
  ...
  let imageUrl: string; let animationUrl: string | undefined
  let files: Array<{ uri: string; type: string }>; let category: string
  ...
  if (hasMultipleAssets && post.assets) {
    ...
    imageUrl = post.coverUrl ? post.coverUrl : (post.protectDownload && isDocumentType) ? post.coverUrl! : post.mediaUrl
    ...
    const mediaUri = post.protectDownload && post.assetId ? `https://www.desperse.com/api/assets/${post.assetId}` : post.mediaUrl
    files = [{ uri: mediaUri, type: mediaMime }, ...(post.coverUrl ? [{ uri: post.coverUrl, type: coverMime || 'image/png' }] : [])]
    animationUrl = post.coverUrl ? (post.protectDownload && post.assetId ? `https://www.desperse.com/api/assets/${post.assetId}` : post.mediaUrl) : undefined
  }

  return {
    name, symbol, description, image: imageUrl, animation_url: animationUrl,
    ...
    attributes: [
      ...
      ...(post.categories?.length ? post.categories.map(cat => ({ trait_type: 'Category', value: cat.display })) : []),
    ],
